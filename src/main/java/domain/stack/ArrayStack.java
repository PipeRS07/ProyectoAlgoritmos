...
public class ArrayStack implements Stack {
    ...
    public Object searchName(Object evaluador) throws StackException {
        ArrayStack aux = new ArrayStack(this.n);
        ArrayStack result = new ArrayStack(n);
        while(!isEmpty()){

            if(util.Utility.compare(evaluador, peek())==0)
            aux.push(pop());
        }
    ...
    @Override
    public String toString() {//Para no perder la funcionalidad no se usa for
        if(isEmpty()) return "Array Stack is Empty";
        String result = "\nArray Stack Content";
        try {//Vaciamos la pila
